...
builder.Services.AddServerSideBlazor()
    .AddHubOptions(options =>
    {
        ...
        options.HandshakeTimeout = TimeSpan.FromSeconds(30); // 获取或设置服务器用于客户端传入握手请求超时的间隔
        options.KeepAliveInterval = TimeSpan.FromSeconds(30); // 获取或设置串行器用于向连接的客户端发送keep alive ping的间隔
