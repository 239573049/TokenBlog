...
builder.Services.AddServerSideBlazor()
    ...
    {
        options.HandshakeTimeout = TimeSpan.FromSeconds(60000000); // 获取或设置服务器用于客户端传入握手请求超时的间隔
        options.ClientTimeoutInterval = TimeSpan.FromSeconds(60000000); // 长时间未发送消息断开
        ...
        options.MaximumParallelInvocationsPerClient = 2; // 默认情况下，客户端一次只允许调用一个Hub方法。更改此属性将允许客户端在排队之前同时调用多个方法。
