...
builder.Services.AddServerSideBlazor()
    ...
    {
        options.ClientTimeoutInterval = TimeSpan.FromSeconds(120); // 长时间未发送消息断开
        options.EnableDetailedErrors = false; // 获取或设置一个值，该值指示是否将详细错误消息发送到客户端。详细的错误消息包括来自服务器上抛出的异常的详细信息。
        options.HandshakeTimeout = TimeSpan.FromSeconds(60); // 获取或设置服务器用于客户端传入握手请求超时的间隔
        options.KeepAliveInterval = TimeSpan.FromSeconds(60); // 获取或设置串行器用于向连接的客户端发送keep alive ping的间隔
